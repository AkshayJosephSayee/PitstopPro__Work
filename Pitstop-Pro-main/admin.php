...
<body>
    ...
    <script>
        ...
        async function loadBillBookings(){
            ...
            sel.innerHTML = '<option value="">Select a booking</option>' + bookings.filter(b=> (b.b_status||b.status)==='completed').map(b=>`<option value="${b.booking_id||b.id}">${b.booking_id||b.id} - ${b.Username||b.customer} - ${b.service_type||b.service}</option>`).join('');
            billItems = [];
            document.getElementById('additionalItems').innerHTML = '';
            updateBillPreview();
        }

        function loadBookingDetails(){ const id=document.getElementById('billBooking').value; const b = bookings.find(x => (x.booking_id||x.id)==id); if(!b) return; document.getElementById('billCustomerName').value = b.Username||b.customer||''; document.getElementById('billVehicle').value = b.Vehicle||b.vehicle||''; document.getElementById('billService').value = b.service_type||b.service||''; updateBillPreview(); }
        ...
        async function generateBill(){ const bookingId=document.getElementById('billBooking').value; if(!bookingId){alert('Please select a booking'); return;} const items = billItems.map(i=>({ name: document.getElementById('itemName-'+i.id)?.value || '', price: parseFloat(document.getElementById('itemPrice-'+i.id)?.value) || 0 })).filter(it=>it.name && it.price>0); try{ const res = await fetch('admin_ajax.php', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: `action=generateBill&bookingId=${encodeURIComponent(bookingId)}&items=${encodeURIComponent(JSON.stringify(items))}` }); const j = await res.json(); if(j.success){ alert('Bill generated. Total: ' + (j.data?.total || j.data?.total_amount || 'N/A')); window.print(); } else alert(j.error || 'Failed'); }catch(e){console.error(e);} }
